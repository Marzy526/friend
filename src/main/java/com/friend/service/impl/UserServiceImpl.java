...
import com.friend.utils.AlgorithmUtils;
import com.google.gson.Gson;
import com.google.gson.reflect.TypeToken;
import com.friend.constant.UserConstant;
...
import static com.friend.constant.UserConstant.USER_LOGIN_STATE;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public List<User> matchUsers(long num, User loginUser) {
        ...
        // 此时的User只有id，tags属性，获取id后重新查询完整User
        List<Long> userIdList = topUserPairList.stream().map(pair -> pair.getKey().getId()).collect(Collectors.toList());
        QueryWrapper<User> userQueryWrapper = new QueryWrapper<>();
        userQueryWrapper.in("id", userIdList);
        return this.list(userQueryWrapper).stream().map(user -> getSafetyUser(user)).collect(Collectors.toList());
    }
